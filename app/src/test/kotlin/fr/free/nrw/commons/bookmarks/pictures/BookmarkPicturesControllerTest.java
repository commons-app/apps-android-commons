...
public class BookmarkPicturesControllerTest {
  ...
  @Mock BookmarkPicturesDao bookmarkDao;

  @InjectMocks BookmarkPicturesController bookmarkPicturesController;
  ...
  public void setup() {
    ...
    when(bookmarkDao.getAllBookmarks()).thenReturn(getMockBookmarkList());
    when(mediaClient.getMedia(anyString())).thenReturn(Single.just(mockMedia));
  }
  ...
  public void loadBookmarkedPicturesForNullMedia() {
    ...
    when(mediaClient.getMedia("File:Test2.jpg")).thenReturn(Single.just(getMockMedia()));
    List<Media> bookmarkedPictures =
        bookmarkPicturesController.loadBookmarkedPictures().blockingGet();
