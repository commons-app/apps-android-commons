...
public class ContributionViewHolder extends RecyclerView.ViewHolder {
  ...
  @BindView(R.id.failed_image_options)
  ...
  /**
   * This method fetches the thumbnail url from file name If the thumbnail url is present in cache,
   * then it is used otherwise API call is made to fetch the thumbnail This can be removed once
   * #2904 is in place and contribution contains all metadata beforehand
   *
   * @param contribution
   */
  private void fetchAndDisplayThumbnail(DisplayableContribution contribution) {
    ...
    if ((contribution.getState() != Contribution.STATE_COMPLETED)
        && FileUtils.fileExists(contribution.getLocalUri())) {
