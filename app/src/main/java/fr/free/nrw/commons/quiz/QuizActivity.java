...
public class QuizActivity extends AppCompatActivity {
  ...
  @BindView(R.id.quiz_positive_answer)
  RadioButton positiveAnswer;

  @BindView(R.id.quiz_negative_answer)
  ...
  public void setNextQuestion() {
    if (questionIndex <= quiz.size()
        && (positiveAnswer.isChecked() || negativeAnswer.isChecked())) {
      evaluateScore();
  ...
  public void displayQuestion() {
    ...
    questionTitle.setText(
        getResources().getString(R.string.question) + quiz.get(questionIndex).getQuestionNumber());
    imageView.setHierarchy(
        GenericDraweeHierarchyBuilder.newInstance(getResources())
            .setFailureImage(
                VectorDrawableCompat.create(
                    getResources(), R.drawable.ic_error_outline_black_24dp, getTheme()))
  ...
  public void evaluateScore() {
    if ((quiz.get(questionIndex).isAnswer() && positiveAnswer.isChecked())
      ...
      customAlert(
          getResources().getString(R.string.correct), quiz.get(questionIndex).getAnswerMessage());
      score++;
    } else {
      customAlert(
          getResources().getString(R.string.wrong), quiz.get(questionIndex).getAnswerMessage());
    }
  }
