...
public class MainActivity extends NavigationBaseActivity
  ...
  @Inject public LocationServiceManager locationManager;
  @Inject NotificationController notificationController;
  @Inject QuizChecker quizChecker;
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    setTitle(
        getString(
            R.string
                .navigation_item_home)); // Should I create a new string variable with another name
    // instead?
    ...
    if (savedInstanceState != null) {
      onOrientationChanged =
          true; // Will be used in nearby fragment to determine significant update of map
  ...
  private void initMain() {
    ...
    ContentResolver.setSyncAutomatically(
        sessionManager.getCurrentAccount(), BuildConfig.CONTRIBUTION_AUTHORITY, true);
    requestSync(
        sessionManager.getCurrentAccount(), BuildConfig.CONTRIBUTION_AUTHORITY, new Bundle());
  ...
  private void addTabsAndFragments() {
    contributionsActivityPagerAdapter =
        new ContributionsActivityPagerAdapter(getSupportFragmentManager());
    viewPager.setAdapter(contributionsActivityPagerAdapter);

    tabLayout.addTab(
        tabLayout.newTab().setText(getResources().getString(R.string.contributions_fragment)));
    tabLayout.addTab(
        tabLayout.newTab().setText(getResources().getString(R.string.nearby_fragment)));

    // Set custom view to add nearby info icon next to text
    View nearbyTabLinearLayout =
        LayoutInflater.from(this).inflate(R.layout.custom_nearby_tab_layout, null);
    ...
    ((ContributionsFragment) contributionsActivityPagerAdapter.getItem(CONTRIBUTIONS_TAB_POSITION))
  ...
  public void setNumOfUploads(int uploadCount) {
    tabLayout
        .getTabAt(0)
        .setText(
            getResources().getString(R.string.contributions_fragment)
                + " "
                + getResources()
  ...
  /**
   * Normally tab layout and view pager has no relation, which means when you swipe view pager tab
   * won't change and vice versa. So we have to notify each of them.
   */
  ...
  public void onBackPressed() {
    ...
    String contributionsFragmentTag =
        ((ContributionsActivityPagerAdapter) viewPager.getAdapter())
            .makeFragmentName(R.id.pager, 0);
    String nearbyFragmentTag =
        ((ContributionsActivityPagerAdapter) viewPager.getAdapter())
            .makeFragmentName(R.id.pager, 1);
    if (drawer.isDrawerOpen(GravityCompat.START)) {
      drawer.closeDrawer(GravityCompat.START);
    } else if (getSupportFragmentManager().findFragmentByTag(contributionsFragmentTag) != null
      ...
      ContributionsFragment contributionsFragment =
          (ContributionsFragment)
              getSupportFragmentManager().findFragmentByTag(contributionsFragmentTag);

      if (contributionsFragment
              .getChildFragmentManager()
              .findFragmentByTag(ContributionsFragment.MEDIA_DETAIL_PAGER_FRAGMENT_TAG)
          != null) {
        // Means that media details fragment is visible to uer instead of contributions list
        // fragment (As chils fragment)
        // Then we want to go back to contributions list fragment on backbutton pressed from media
        // detail fragment
        contributionsFragment.getChildFragmentManager().popBackStack();
        // Tabs were invisible when Media Details Fragment is active, make them visible again on
        // Contrib List Fragment active
        showTabs();
        // Nearby Notification Card View was invisible when Media Details Fragment is active, make
        // it visible again on Contrib List Fragment active, according to preferences
        if (defaultKvStore.getBoolean("displayNearbyCardView", true)) {
          if (contributionsFragment.nearbyNotificationCardView.cardViewVisibilityState
  ...
  /**
   * Responsible with displaying required menu items according to displayed fragment. Notifications
   * icon when contributions list is visible, list sheet icon when nearby is visible
   */
    ...
    switch (item.getItemId()) {
      ...
      case R.id.list_sheet:
        if (contributionsActivityPagerAdapter.getItem(1) != null) {
          ((NearbyParentFragment) contributionsActivityPagerAdapter.getItem(1))
  ...
  public class ContributionsActivityPagerAdapter extends FragmentPagerAdapter {
    ...
    /**
     * A simple hack to use retained fragment when getID is called explicitly, if we don't use this
     * method, a new fragment will be initialized on each explicit calls of getID
     *
