...
public class ReviewHelper {
  ...
  /**
   * Fetches recent changes from MediaWiki API Calls the API to get 10 changes in the last 1 hour
   * Earlier we were getting changes for the last 30 days but as the API returns just 10 results its
   * best to fetch for just last 1 hour.
   *
   * @return
   */
  ...
  /**
   * Gets a random file change for review. - Picks the most recent changes in the last 30 day window
   * - Picks a random file from those changes - Checks if the file is nominated for deletion -
  ...
  /**
   * Returns a proper Media object if the file is not already nominated for deletion Else it returns
   * an empty Media object
   *
   * @param recentChange
   * @return
   */
  private Single<Media> getRandomMediaFromRecentChange(RecentChange recentChange) {
    return Single.just(recentChange)
        .flatMap(
            change ->
                mediaClient.checkPageExistsUsingTitle(
                    "Commons:Deletion_requests/" + change.getTitle()))
        .flatMap(
            isDeleted -> {
              if (isDeleted) {
