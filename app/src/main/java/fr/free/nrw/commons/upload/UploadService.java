...
public class UploadService extends HandlerService<Contribution> {
  ...
  protected class NotificationUpdateProgressListener {
    ...
    public void onProgress(long transferred, long total) {
      ...
      if (transferred == total) {
        ...
        curNotification.setProgress(
            100, (int) (((double) transferred / (double) total) * 100), false);
      }
      notificationManager.notify(
          notificationTag, NOTIFICATION_UPLOAD_IN_PROGRESS, curNotification.build());
  ...
  public void queue(int what, Contribution contribution) {
    Timber.d(
        "Upload service queue has contribution with wiki data entity id as %s",
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    if (ACTION_START_SERVICE.equals(intent.getAction()) && freshStart) {
      ...
      int updated =
          getContentResolver()
              .update(
                  ContributionsContentProvider.BASE_URI,
                  failedValues,
                  ContributionDao.Table.COLUMN_STATE
                      ...
                      + " = ?",
                  new String[] {
                    String.valueOf(Contribution.STATE_QUEUED),
  ...
  private void uploadContribution(Contribution contribution) {
    ...
    curNotification
        .setContentTitle(
            getString(
                R.string.upload_progress_notification_title_start, contribution.getDisplayTitle()))
        .setContentText(
            getResources()
                .getQuantityString(
                    R.plurals.uploads_pending_notification_indicator, toUpload, toUpload))
        .setTicker(
            getString(
                R.string.upload_progress_notification_title_in_progress,
    ...
    NotificationUpdateProgressListener notificationUpdater =
        new NotificationUpdateProgressListener(
            notificationTag,
            getString(
                R.string.upload_progress_notification_title_in_progress,
                contribution.getDisplayTitle()),
    ...
    Observable.fromCallable(() -> "Temp_" + contribution.hashCode() + filename)
        .flatMap(
            stashFilename ->
                uploadClient.uploadFileToStash(
                    getApplicationContext(), stashFilename, localFile, notificationUpdater))
        ...
        .subscribe(
            uploadResult -> {
              ...
              if (!resultStatus.equals("Success")) {
                ...
                Timber.d(
                    "Contribution upload success. Initiating Wikidata edit for entity id %s",
                    contribution.getWikiDataEntityId());
                wikidataEditService.createClaimWithLogging(
                    contribution.getWikiDataEntityId(), canonicalFilename);
                contribution.setFilename(canonicalFilename);
                contribution.setImageUrl(uploadResult.getImageinfo().getOriginalUrl());
                contribution.setState(Contribution.STATE_COMPLETED);
                contribution.setDateUploaded(
                    CommonsDateUtil.getIso8601DateFormatShort()
  ...
  private void showFailedNotification(Contribution contribution) {
    ...
    notificationManager.notify(
        contribution.getLocalUri().toString(), NOTIFICATION_UPLOAD_FAILED, curNotification.build());
