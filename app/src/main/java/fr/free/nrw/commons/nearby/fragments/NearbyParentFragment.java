...
public class NearbyParentFragment extends CommonsDaggerSupportFragment
  ...
  private void addCheckBoxCallback() {
    checkBoxTriStates
        .setCallback((o, state, b, b1) -> presenter.filterByMarkerType(o, state, b, b1));
  }
      ...
      checkPermissionsAndPerformAction(() -> {
        ...
        if (lastKnownLocation != null) {
          final CameraPosition position = new CameraPosition.Builder()
              .target(
                  LocationUtils.commonsLatLngToMapBoxLatLng(target)) // Sets the new camera position
  ...
  public void initNearbyFilter() {
    ...
    nearbyFilterSearchRecyclerViewAdapter = new NearbyFilterSearchRecyclerViewAdapter(getContext(),
        new ArrayList<>(Label.valuesAsList()), recyclerView);
    nearbyFilterSearchRecyclerViewAdapter
        .setCallback(new NearbyFilterSearchRecyclerViewAdapter.Callback() {
          @Override
    ...
    nearbyFilterList.getLayoutParams().width = (int) LayoutUtils
        .getScreenWidth(getActivity(), 0.75);
  ...
  public boolean isCurrentLocationMarkerVisible() {
    if (latLngBounds == null) {
      ...
      Timber.d("Current location marker %s",
          latLngBounds.contains(currentLocationMarker.getPosition()) ? "visible" : "invisible");
  ...
  private void populatePlacesForCurrentLocation(final fr.free.nrw.commons.location.LatLng curlatLng,
    ...
    compositeDisposable.add(Observable.fromCallable(() -> nearbyController
        ...
        .subscribe(nearbyPlacesInfo -> {
            ...
            throwable -> {
              Timber.d(throwable);
              showErrorMessage(getString(R.string.error_fetching_nearby_places) + throwable
                  .getLocalizedMessage());
              setProgressBarVisibility(false);
  ...
  private void showFABs() {
    ...
    fabPlus.show();
    NearbyFABUtils
        .addAnchorToSmallFABs(fabGallery, getView().findViewById(R.id.empty_view).getId());
    NearbyFABUtils
        .addAnchorToSmallFABs(fabCamera, getView().findViewById(R.id.empty_view1).getId());
  }
  ...
  private boolean isUserBrowsing() {
    final boolean isUserBrowsing = lastKnownLocation != null && !presenter
        .areLocationsClose(getCameraTarget(), lastKnownLocation);
  ...
  public void onLocationChangedSlightly(final fr.free.nrw.commons.location.LatLng latLng) {
    ...
    if (isMapBoxReady && latLng != null
        && !isUserBrowsing()) {//If the map has never ever shown the current location, lets do it know
  ...
  public void addCurrentLocationMarker(final fr.free.nrw.commons.location.LatLng curLatLng) {
    if (null != curLatLng) {
      ExecutorUtils.get().submit(() -> {
        ...
        mapView
            .post(() -> currentLocationPolygon = mapBox.addPolygon(currentLocationPolygonOptions));
   ...
   * more than grey(as the name of the icon might suggest) since grey icon may lead the users to
   * believe that it is disabled or prohibited contribution
   */
  private void hideAllMarkers() {
    final VectorDrawableCompat vectorDrawable;
    vectorDrawable = VectorDrawableCompat.create(
        getContext().getResources(), R.drawable.ic_custom_greyed_out_marker,
        getContext().getTheme());
    final Bitmap icon = UiUtils.getBitmap(vectorDrawable);
  ...
  private void addNearbyMarkersToMapBoxMap(final List<NearbyBaseMarker> nearbyBaseMarkers,
      final Marker selectedMarker) {
