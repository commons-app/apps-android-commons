...
public class NearbyParentFragment extends CommonsDaggerSupportFragment
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    mapView.getMapAsync(
        mapBoxMap -> {
          ...
          initNearbyFilter();
          mapBoxMap.setStyle(
              isDarkTheme ? Style.DARK : Style.OUTDOORS,
              style -> {
      ...
      checkPermissionsAndPerformAction(
          () -> {
            ...
            if (lastKnownLocation != null) {
              ...
              Toast.makeText(
                      getContext(),
                      getString(R.string.nearby_location_not_available),
                      Toast.LENGTH_LONG)
                  .show();
            }
  ...
  public void onPause() {
    ...
    try {
      ...
      Timber.e(e);
      // Broadcast receivers should always be unregistered inside catch, you never know if they were
  ...
  public void initNearbyFilter() {
    ...
    nearbyFilterSearchRecyclerViewAdapter =
        new NearbyFilterSearchRecyclerViewAdapter(
            getContext(), new ArrayList<>(TEXT_TO_DESCRIPTION.values()), recyclerView);
    nearbyFilterSearchRecyclerViewAdapter.setCallback(
        new NearbyFilterSearchRecyclerViewAdapter.Callback() {
          ...
          @Override
          public void filterByMarkerType(
              ArrayList<Label> selectedLabels, int i, boolean b, boolean b1) {
    ...
    nearbyFilterList.getLayoutParams().width =
        (int) LayoutUtils.getScreenWidth(getActivity(), 0.75);
    ...
    compositeDisposable.add(
        RxSearchView.queryTextChanges(searchView)
            ...
            .subscribe(
                query -> {
                  ((NearbyFilterSearchRecyclerViewAdapter) recyclerView.getAdapter())
                      .getFilter()
  ...
  private void initFilterChips() {
    chipNeedsPhoto.setOnCheckedChangeListener(
    ...
    chipExists.setOnCheckedChangeListener(
        (buttonView, isChecked) -> {
          if (NearbyController.currentLocation != null) {
            checkBoxTriStates.setState(CheckBoxTriStates.UNKNOWN);
            if (isChecked) {
  ...
  private void setBottomSheetCallbacks() {
    bottomSheetDetailsBehavior.setBottomSheetCallback(
    ...
    bottomSheetDetails.setOnClickListener(
        v -> {
    ...
    rlBottomSheet.getLayoutParams().height =
    ...
    bottomSheetListBehavior.setBottomSheetCallback(
        new BottomSheetBehavior.BottomSheetCallback() {
          @Override
          public void onStateChanged(@NonNull View bottomSheet, int newState) {
  ...
  public void updateListFragment(List<Place> placeList) {
    adapterFactory.updateAdapterData(
        placeList, (RVRendererAdapter<Place>) rvNearbyList.getAdapter());
  }
  ...
  public void populatePlaces(fr.free.nrw.commons.location.LatLng curlatLng) {
    ...
    if (curlatLng.equals(lastFocusLocation)
        || lastFocusLocation == null) { // Means we are checking around current location
  ...
  private void populatePlacesForAnotherLocation(
    ...
    compositeDisposable.add(
        Observable.fromCallable(
                () ->
                    nearbyController.loadAttractionsFromLocation(
                        curlatLng, searchLatLng, false, false))
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe(
                nearbyPlacesInfo -> {
  ...
  /**
   * Populates places for your location, should be used for finding nearby places around a location
   * where you are.
   *
   * @param nearbyPlacesInfo This variable has place list information and distances.
   */
  private void updateMapMarkers(
      NearbyController.NearbyPlacesInfo nearbyPlacesInfo, boolean shouldUpdateSelectedMarker) {
  ...
  private void showFABs() {
    ...
    fabPlus.show();
    NearbyFABUtils.addAnchorToSmallFABs(
        fabGallery, getView().findViewById(R.id.empty_view).getId());
    NearbyFABUtils.addAnchorToSmallFABs(
        fabCamera, getView().findViewById(R.id.empty_view1).getId());
  }
  ...
  private void handleLocationUpdate(
      fr.free.nrw.commons.location.LatLng latLng,
  ...
  private boolean isUserBrowsing() {
    boolean isUserBrowsing =
        lastKnownLocation != null
            && !presenter.areLocationsClose(getCameraTarget(), lastKnownLocation);
  ...
  public void onLocationChangedSlightly(fr.free.nrw.commons.location.LatLng latLng) {
    ...
    if (isMapBoxReady
        && latLng != null
        && !isUserBrowsing()) { // If the map has never ever shown the current location, lets do it
      // know
  ...
  void setMapBoxPosition(fr.free.nrw.commons.location.LatLng latLng) {
    CameraPosition position =
        new CameraPosition.Builder()
            .target(
                LocationUtils.commonsLatLngToMapBoxLatLng(latLng)) // Sets the new camera position
            .zoom(ZOOM_LEVEL) // Same zoom level
            .build();
    mapBox.moveCamera(CameraUpdateFactory.newCameraPosition(position));
  }
   ...
   *
   * <p>Should be called only on creation of mapboxMap, there is other method to update markers
      ...
      ExecutorUtils.get()
          .submit(
              () -> {
                ...
                List<LatLng> circle =
                    UiUtils.createCircleArray(
                        curLatLng.getLatitude(),
                        curLatLng.getLongitude(),
                        curLatLng.getAccuracy() * 2,
                        100);

                PolygonOptions currentLocationPolygonOptions =
                    new PolygonOptions()
                        .addAll(circle)
  ...
  public void updateMapToTrackPosition(fr.free.nrw.commons.location.LatLng curLatLng) {
    ...
    CameraPosition cameraPosition =
        new CameraPosition.Builder()
            .target(LocationUtils.commonsLatLngToMapBoxLatLng(curLatLng))
            .build();
    if (null != mapBox) {
      mapBox.setCameraPosition(cameraPosition);
      mapBox.animateCamera(CameraUpdateFactory.newCameraPosition(cameraPosition), 1000);
    }
  }
   ...
   */
  @Override
  public void filterMarkersByLabels(
      ...
      for (MarkerPlaceGroup markerPlaceGroup : NearbyController.markerLabelList) {
        if (displayExists && displayNeedsPhoto) {
          ...
          if (markerPlaceGroup.getPlace().destroyed.trim().isEmpty()
              && markerPlaceGroup.getPlace().pic.trim().isEmpty()) {
            updateMarker(
                markerPlaceGroup.getIsBookmarked(),
                markerPlaceGroup.getPlace(),
                NearbyController.currentLocation);
  ...
  public fr.free.nrw.commons.location.LatLng getCameraTarget() {
    return mapBox == null
        ? null
        : LocationUtils.mapBoxLatLngToCommonsLatLng(mapBox.getCameraPosition().target);
  }
  ...
  public void updateMarker(
      boolean isBookmarked, Place place, @Nullable fr.free.nrw.commons.location.LatLng curLatLng) {
    VectorDrawableCompat vectorDrawable;
    if (isBookmarked) {
      vectorDrawable =
          VectorDrawableCompat.create(
              getContext().getResources(),
              R.drawable.ic_custom_bookmark_marker,
              getContext().getTheme());
    } else if (!place.pic.trim().isEmpty()) {
      vectorDrawable =
          VectorDrawableCompat.create( // Means place has picture
              getContext().getResources(),
  ...
  private void greyOutAllMarkers() {
    ...
    vectorDrawable =
        VectorDrawableCompat.create(
            getContext().getResources(),
            R.drawable.ic_custom_greyed_out_marker,
            getContext().getTheme());
    Bitmap icon = UiUtils.getBitmap(vectorDrawable);
    for (Marker marker : mapBox.getMarkers()) {
  ...
  private void addNearbyMarkersToMapBoxMap(
      List<NearbyBaseMarker> nearbyBaseMarkers, Marker selectedMarker) {
  ...
  private void setMapMarkerActions(Marker selectedMarker) {
    if (mapBox != null) {
      mapBox.setOnInfoWindowCloseListener(
          marker -> {
      ...
      mapBox.setOnMarkerClickListener(
          marker -> {
  ...
  public void recenterMap(fr.free.nrw.commons.location.LatLng curLatLng) {
    ...
    if (ViewUtil.isPortrait(getActivity())) {
      position =
          new CameraPosition.Builder()
              .target(
                  isListBottomSheetExpanded()
                      ? new LatLng(
                          curLatLng.getLatitude() - CAMERA_TARGET_SHIFT_FACTOR_PORTRAIT,
                          curLatLng.getLongitude())
                      : new LatLng(
                          curLatLng.getLatitude(),
                          curLatLng.getLongitude(),
      ...
      position =
          new CameraPosition.Builder()
              .target(
                  isListBottomSheetExpanded()
                      ? new LatLng(
                          curLatLng.getLatitude() - CAMERA_TARGET_SHIFT_FACTOR_LANDSCAPE,
                          curLatLng.getLongitude())
                      : new LatLng(
                          curLatLng.getLatitude(),
                          curLatLng.getLongitude(),
  ...
  /**
   * If nearby details bottom sheet state is collapsed: show fab plus If nearby details bottom sheet
   * state is expanded: show fab plus If nearby details bottom sheet state is hidden: hide all fabs
   *
  ...
  private void passInfoToSheet(Place place) {
    ...
    wikipediaButton.setOnClickListener(
        view -> Utils.handleWebUrl(getContext(), this.selectedPlace.siteLinks.getWikipediaLink()));

    wikidataButton.setVisibility(place.hasWikidataLink() ? View.VISIBLE : View.GONE);
    wikidataButton.setOnClickListener(
        view -> Utils.handleWebUrl(getContext(), this.selectedPlace.siteLinks.getWikidataLink()));

    directionsButton.setOnClickListener(
        view -> Utils.handleGeoCoordinates(getActivity(), this.selectedPlace.getLocation()));

    commonsButton.setVisibility(this.selectedPlace.hasCommonsLink() ? View.VISIBLE : View.GONE);
    commonsButton.setOnClickListener(
        view -> Utils.handleWebUrl(getContext(), this.selectedPlace.siteLinks.getCommonsLink()));
