...
public interface MediaInterface {
  ...
  Observable<MwQueryResponse> getMediaListFromCategory(
      @Query("gcmtitle") String category,
      @Query("gcmlimit") int itemLimit,
  ...
  @GET(
      "w/api.php?action=query&format=json&formatversion=2"
          + // Basic parameters
          "&generator=search&gsrwhat=text&gsrnamespace=6"
          + // Search parameters
          MEDIA_PARAMS)
  Observable<MwQueryResponse> getMediaListFromSearch(
      @Query("gsrsearch") String keyword,
