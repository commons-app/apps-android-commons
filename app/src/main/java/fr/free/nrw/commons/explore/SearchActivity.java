...
public class SearchActivity extends NavigationBaseActivity
  ...
  /**
   * This method is called on success of API call for image Search. The viewpager will notified that
   * number of items have changed.
   */
  ...
  protected void onResume() {
    if (supportFragmentManager.getBackStackEntryCount() == 1) {
      // FIXME: Temporary fix for screen rotation inside media details. If we don't call
      // onBackPressed then fragment stack is increasing every time.
      // FIXME: Similar issue like this
      // https://github.com/commons-app/apps-android-commons/issues/894
      // This is called on screen rotation when user is inside media details. Ideally it should show
      // Media Details but since we are not saving the state now. We are throwing the user to search
      // screen otherwise the app was crashing.
      //
      onBackPressed();
    }
    super.onResume();
  ...
  public void updateText(String query) {
    ...
    // Clear focus of searchView now. searchView.clearFocus(); does not seem to work Check the below
   ...
   * query and adds it to the recycler view and viewpager adapter
   */
