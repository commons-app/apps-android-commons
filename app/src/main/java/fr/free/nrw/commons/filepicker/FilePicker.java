...
public class FilePicker implements Constants {
  ...
  private static Intent createGalleryIntent(@NonNull Context context, int type) {
    ...
    return plainGalleryPickerIntent()
        .putExtra(
            Intent.EXTRA_ALLOW_MULTIPLE, configuration(context).allowsMultiplePickingInGallery());
  }
  ...
  private static Intent createCameraForImageIntent(@NonNull Context context, int type) {
    ...
    try {
      ...
      // We have to explicitly grant the write permission since Intent.setFlag works only on API
      // Level >=20
  ...
  private static void onPictureReturnedFromDocuments(
      Intent data, Activity activity, @NonNull FilePicker.Callbacks callbacks) {
    try {
      Uri photoPath = data.getData();
      UploadableFile photoFile = PickedFiles.pickedExistingPicture(activity, photoPath);
      callbacks.onImagesPicked(
          singleFileList(photoFile), FilePicker.ImageSource.DOCUMENTS, restoreType(activity));
  ...
  private static List<UploadableFile> getFilesFromGalleryPictures(Intent data, Activity activity)
      throws IOException {
  ...
  private static void onVideoReturnedFromCamera(
      Activity activity, @NonNull FilePicker.Callbacks callbacks) {
    try {
      String lastVideoUri =
          PreferenceManager.getDefaultSharedPreferences(activity).getString(KEY_VIDEO_URI, null);
  ...
  public interface Callbacks {
    ...
    void onImagesPicked(
        @NonNull List<UploadableFile> imageFiles, FilePicker.ImageSource source, int type);
