...
public class ImageUtils {
  ...
  /**
   * The parameter FILE_FBMD is returned from the class ReadFBMD if the uploaded image contains FBMD
   * data else returns IMAGE_OK ie. 10000
   */
  public static final int FILE_FBMD = 1 << 4;
  /**
   * The parameter FILE_NO_EXIF is returned from the class EXIFReader if the uploaded image does not
   * contains EXIF data else returns IMAGE_OK ie. 100000
   */
   ...
   * @return false if image is neither dark nor blurry or if the input bitmapRegionDecoder provided
   *     is null true if geolocation of the image and wikidata item are different
   */
  static boolean checkImageGeolocationIsDifferent(String geolocationOfFileString, LatLng latLng) {
    ...
    Double distance =
        LengthUtils.computeDistanceBetween(
            new LatLng(
                Double.parseDouble(geolocationOfFile[0]),
                Double.parseDouble(geolocationOfFile[1]),
                0),
            latLng);
  ...
  public static void setWallpaperFromImageUrl(Context context, Uri imageUrl) {
    ...
    ImageRequest imageRequest =
        ImageRequestBuilder.newBuilderWithSource(imageUrl).setAutoRotateEnabled(true).build();
  ...
  /**
   * Result variable is a result of an or operation of all possible problems. Ie. if result is 0001
   * means IMAGE_DARK if result is 1100 IMAGE_DUPLICATE and IMAGE_GEOLOCATION_DIFFERENT
   */
    ...
    if (result <= 0) {
      ...
      if ((IMAGE_GEOLOCATION_DIFFERENT & result) != 0) {
        errorMessage
            .append("\n - ")
            .append(
                context.getResources().getString(R.string.upload_problem_different_geolocation));
      }
