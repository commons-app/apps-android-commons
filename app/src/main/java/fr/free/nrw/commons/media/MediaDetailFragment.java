...
public class MediaDetailFragment extends CommonsDaggerSupportFragment {
  ...
  @Inject MediaDataExtractor mediaDataExtractor;
  @Inject ReasonBuilder reasonBuilder;
  @Inject DeleteHelper deleteHelper;
  @Inject ViewUtilWrapper viewUtil;
  ...
  private ViewTreeObserver.OnGlobalLayoutListener
      layoutListener; // for layout stuff, only used once!
  private ViewTreeObserver.OnScrollChangedListener scrollListener;

  // Had to make this class variable, to implement various onClicks, which access the media, also I
  // fell why make separate variables when one can serve the purpose
  private Media media;
  ...
  public void onResume() {
    ...
    if (getParentFragment() != null && getParentFragment().getParentFragment() != null) {
      // Added a check because, not necessarily, the parent fragment will have a parent fragment,
      // say
      // in the case when MediaDetailPagerFragment is directly started by the CategoryImagesActivity
      ((ContributionsFragment) (getParentFragment().getParentFragment()))
  ...
  private void displayMediaDetails() {
    ...
    Disposable disposable =
        mediaDataExtractor
            .fetchMediaDetails(media.getFilename())
  ...
  public void onDestroyView() {
    if (layoutListener != null && getView() != null) {
      getView()
          .getViewTreeObserver()
          .removeGlobalOnLayoutListener(layoutListener); // old Android was on crack. CRACK IS WHACK
      layoutListener = null;
    }
  ...
  public void onDeleteButtonClicked() {
    if (AccountUtil.getUserName(getContext()).equals(media.getCreator())) {
      final ArrayAdapter<String> languageAdapter =
          new ArrayAdapter<>(getActivity(), R.layout.simple_spinner_dropdown_list, reasonList);
      ...
      builder
          .setTitle(R.string.nominate_delete)
          .setPositiveButton(
              R.string.about_translate_proceed, (dialog, which) -> onDeleteClicked(spinner));
      builder.setNegativeButton(
          R.string.about_translate_cancel, (dialog, which) -> dialog.dismiss());
  ...
  private void onDeleteClicked(Spinner spinner) {
    ...
    Single<Boolean> resultSingle =
        reasonBuilder
            .getReason(media, reason)
  ...
  private void onDeleteClickeddialogtext(String reason) {
    Single<Boolean> resultSingletext =
        reasonBuilder
            .getReason(media, reason)
            .flatMap(reasonString -> deleteHelper.makeDeletion(getContext(), media, reason));
    compositeDisposable.add(
  ...
  private void rebuildCatList() {
    ...
    // As per issue #1826(see https://github.com/commons-app/apps-android-commons/issues/1826), some
    // categories come suffixed with strings prefixed with |. As per the discussion
