...
public class ReviewActivity extends NavigationBaseActivity {
  ...
  @Inject ReviewHelper reviewHelper;
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    d[2].setColorFilter(
        getApplicationContext().getResources().getColor(R.color.button_blue),
    ...
    if (savedInstanceState != null && savedInstanceState.getParcelable(SAVED_MEDIA) != null) {
      ...
      runRandomizer(); // Run randomizer whenever everything is ready so that a first random image
      // will be added
    }
    ...
    btnSkipImage.setOnTouchListener(
        (view, event) -> {
          if (event.getAction() == MotionEvent.ACTION_UP
              && event.getRawX()
                  >= (btnSkipImage.getRight()
                      - btnSkipImage.getCompoundDrawables()[2].getBounds().width())) {
            showSkipImageInfo();
            return true;
          }
          return false;
