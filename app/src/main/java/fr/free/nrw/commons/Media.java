...
public class Media implements Parcelable {
  ...
  public static Creator<Media> CREATOR =
      new Creator<Media>() {
        @Override
  ...
  public static Media from(MwQueryPage page) {
    ...
    if (metadata == null) {
      Media media =
          new Media(null, imageInfo.getOriginalUrl(), page.title(), "", 0, null, null, null);
  ...
  private static String getArtist(ExtMetadata metadata) {
    try {
      ...
      return artistHtml
          .substring(artistHtml.indexOf("title=\""), artistHtml.indexOf("\">"))
          .replace("title=\"User:", "");
    } catch (Exception ex) {
      return "";
    }
  ...
  public String getDisplayTitle() {
    return filename != null
        ? getPageTitle().getDisplayTextWithoutNamespace().replaceFirst("[.][^.]+$", "")
        : "";
  }
  ...
  /**
   * Sets the categories the file falls under. Does not append: i.e. will clear the current
   * categories and then add the specified ones.
   *
