...
public class WikidataEditService {
  ...
  private static final String COMMONS_APP_EDIT_REASON =
      "Add tag for edits made using Android Commons app";
  ...
  /**
   * Edits the wikidata entity by adding the P18 property to it. Adding the P18 edit requires
   * calling the wikidata API to create a claim against the entity
   *
   * @param wikidataEntityId
   * @param fileName
   */
  ...
  private void editWikidataProperty(String wikidataEntityId, String fileName) {
    ...
    wikidataClient
        .createClaim(wikidataEntityId, propertyValue)
        .flatMap(
            revisionId -> {
              if (revisionId != -1) {
                return wikidataClient.addEditTag(
                    revisionId, COMMONS_APP_TAG, COMMONS_APP_EDIT_REASON);
              }
