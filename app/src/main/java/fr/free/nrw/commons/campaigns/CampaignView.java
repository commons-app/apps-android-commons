...
public class CampaignView extends SwipableCardView {
  ...
  public boolean onSwipe(View view) {
    ...
    ((MainActivity) getContext()).defaultKvStore.putBoolean("displayCampaignsCardView", false);
    ViewUtil.showLongToast(
        getContext(), getResources().getString(R.string.nearby_campaign_dismiss_message));
    return true;
  }
  ...
  public class ViewHolder {
    ...
    public void init() {
      if (campaign != null) {
        ...
        try {
          Date startDate =
              CommonsDateUtil.getIso8601DateFormatShort().parse(campaign.getStartDate());
          Date endDate = CommonsDateUtil.getIso8601DateFormatShort().parse(campaign.getEndDate());
          tvDates.setText(
              String.format(
                  "%1s - %2s",
                  DateUtil.getExtraShortDateString(startDate),
