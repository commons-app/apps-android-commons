...
public class ContributionsFragment extends CommonsDaggerSupportFragment
    implements MediaDetailProvider,
  ...
  /**
   * Since we will need to use parent activity on onAuthCookieAcquired, we have to wait fragment to
   * be attached. Latch will be responsible for this sync.
   */
  private ServiceConnection uploadServiceConnection =
      new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder binder) {
          uploadService =
              (UploadService) ((HandlerService.HandlerServiceLocalBinder) binder).getService();
  ...
  public View onCreateView(
    ...
    if (savedInstanceState != null) {
      mediaDetailPagerFragment =
          (MediaDetailPagerFragment)
              getChildFragmentManager().findFragmentByTag(MEDIA_DETAIL_PAGER_FRAGMENT_TAG);
      contributionsListFragment =
          (ContributionsListFragment)
              getChildFragmentManager().findFragmentByTag(CONTRIBUTION_LIST_FRAGMENT_TAG);
        ...
        .registerFragmentLifecycleCallbacks(
            new FragmentManager.FragmentLifecycleCallbacks() {
              ...
              public void onFragmentResumed(FragmentManager fm, Fragment f) {
                ...
                // If media detail pager fragment is visible, hide the campaigns view [might not be
                // the best way to do, this but yeah, this proves to work for now]
              ...
              public void onFragmentDetached(FragmentManager fm, Fragment f) {
                ...
                // If media detail pager fragment is detached, ContributionsList fragment is gonna
                // be visible, [becomes tightly coupled though]
                if (f instanceof MediaDetailPagerFragment) {
                  fetchCampaigns();
                }
              }
  ...
  private void initFragments() {
    ...
    contributionsListFragment.setCallback(
        new Callback() {
          @Override
  ...
  void onAuthCookieAcquired() {
    // Since we call onAuthCookieAcquired method from onAttach, isAdded is still false. So don't use
    // it

    if (getActivity() != null) { // If fragment is attached to parent activity
      getActivity()
          .bindService(getUploadServiceIntent(), uploadServiceConnection, Context.BIND_AUTO_CREATE);
      isUploadServiceConnected = true;
  ...
  private void setUploadCount() {
    ...
    compositeDisposable.add(
        okHttpJsonApiClient
            ...
            .subscribe(this::displayUploadCount, t -> Timber.e(t, "Fetching upload count failed")));
  }
  ...
  public void onSaveInstanceState(Bundle outState) {
    ...
    boolean mediaDetailsVisible =
        mediaDetailPagerFragment != null && mediaDetailPagerFragment.isVisible();
  ...
  private void checkPermissionsAndShowNearbyCardView() {
    if (PermissionUtils.hasPermission(getActivity(), Manifest.permission.ACCESS_FINE_LOCATION)) {
      onLocationPermissionGranted();
        ...
        && (((MainActivity) getActivity()).viewPager.getCurrentItem()
            == CONTRIBUTIONS_TAB_POSITION)) {
      nearbyNotificationCardView.permissionType =
  ...
  private void displayYouWontSeeNearbyMessage() {
    ViewUtil.showLongToast(
        getActivity(), getResources().getString(R.string.unable_to_display_nearest_place));
  }
