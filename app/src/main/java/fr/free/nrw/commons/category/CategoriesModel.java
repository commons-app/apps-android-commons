...
public class CategoriesModel {
  ...
  public Comparator<CategoryItem> sortBySimilarity(final String filter) {
    ...
    return (firstItem, secondItem) ->
        stringSimilarityComparator.compare(firstItem.getName(), secondItem.getName());
  }
  ...
  public boolean containsYear(String item) {
    ...
    // And if it is an irrelevant category such as Media_needing_categories_as_of_16_June_2017(Issue
    // #750)
    // Check if the year in the form of XX(X)0s is relevant, i.e. in the 2000s or 2010s as stated in
    // Issue #1029
    return ((item.matches(".*(19|20)\\d{2}.*")
            && !item.contains(yearInString)
            && !item.contains(prevYearInString))
        || item.matches("(.*)needing(.*)")
        || item.matches("(.*)taken on(.*)")
  ...
  public Observable<CategoryItem> searchAll(String term, List<String> imageTitleList) {
    ...
    if (TextUtils.isEmpty(term)) {
      Observable<CategoryItem> categoryItemObservable =
          gpsCategories().concatWith(titleCategories(imageTitleList));
    ...
    // term passed as lower case to make search case-insensitive(taking only lower case for
    // everything)
  ...
  private Observable<CategoryItem> titleCategories(List<String> titleList) {
    return Observable.fromIterable(titleList).concatMap(this::getTitleCategories);
  }

  /**
   * Return category for single title title is converted to lower case to make search
   * case-insensitive
   *
   * @param title
   * @return
  ...
  private Observable<CategoryItem> getTitleCategories(String title) {
    return categoryClient
        .searchCategories(title.toLowerCase(), SEARCH_CATS_LIMIT)
        .map(name -> new CategoryItem(name, false));
  }
