...
public class MediaClient {
  ...
  /**
   * Checks if a page exists on Commons The same method can be used to check for file or talk page
   *
  ...
  /**
   * This method takes the category as input and returns a list of  Media objects filtered using
   * image generator query It uses the generator query API to get the images searched using a query,
   * 10 at a time.
   *
  ...
  public Single<List<Media>> getMediaListFromCategory(String category) {
    return responseToMediaList(
        continuationStore.containsKey("category_" + category) ?
            mediaInterface.getMediaListFromCategory(category, 10,
                continuationStore.get("category_" + category)) : //if true
  ...
  /**
   * This method takes a keyword as input and returns a list of  Media objects filtered using image
   * generator query It uses the generator query API to get the images searched using a query, 10 at
   * a time.
   *
  ...
  public Single<List<Media>> getMediaListFromSearch(String keyword) {
    return responseToMediaList(
        continuationStore.containsKey("search_" + keyword) && (
            continuationStore.get("search_" + keyword) != null) ?
            mediaInterface
                .getMediaListFromSearch(keyword, 10, continuationStore.get("search_" + keyword)) :
            //if true
  ...
  private Single<List<Media>> responseToMediaList(Observable<MwQueryResponse> response,
      String key) {
  ...
  public Single<String> getCaptionByWikibaseIdentifier(String wikibaseIdentifier) {
    return mediaDetailInterface
        .getEntityForImage(Locale.getDefault().getLanguage(), wikibaseIdentifier)
   ...
   *
   * @param filename
   * @return a map containing caption and depictions (empty string in the map if no
   * caption/depictions)
   */
