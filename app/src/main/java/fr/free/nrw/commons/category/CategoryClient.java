...
public class CategoryClient {
   ...
   * @param offset Starts returning items from the nth result. If offset is 9, the response starts
   *     with the 9th item of the search result
   * @return
   */
  ...
  /**
   * The method takes categoryName as input and returns a List of Subcategories It uses the
   * generator query API to get the subcategories in a category, 500 at a time.
   *
   * @param categoryName Category name as defined on commons
   * @return Observable emitting the categories returned. If our search yielded "Category:Test",
   *     "Test" is emitted.
   */
  ...
  /**
   * The method takes categoryName as input and returns a List of parent categories It uses the
   * generator query API to get the parent categories of a category, 500 at a time.
   *
   * @param categoryName Category name as defined on commons
   * @return
   */
   ...
   *     "Test" is emitted.
   */
  private Observable<String> responseToCategoryName(
      Observable<MwQueryResponse> responseObservable) {
    return responseObservable
        .flatMap(
            mwQueryResponse -> {
              MwQueryResult query;
              List<MwQueryPage> pages;
              if ((query = mwQueryResponse.query()) == null || (pages = query.pages()) == null) {
                Timber.d("No categories returned.");
                return Observable.empty();
              } else return Observable.fromIterable(pages);
            })
