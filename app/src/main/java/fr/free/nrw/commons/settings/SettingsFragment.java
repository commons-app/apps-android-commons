...
public class SettingsFragment extends PreferenceFragment {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    ApplicationlessInjection.getInstance(getActivity().getApplicationContext())
    ...
    useAuthorName.setOnPreferenceChangeListener(
        (preference, newValue) -> {
          authorName.setEnabled((Boolean) newValue);
          return true;
        });

    MultiSelectListPreference multiSelectListPref =
        (MultiSelectListPreference) findPreference(Prefs.MANAGED_EXIF_TAGS);
    if (multiSelectListPref != null) {
      multiSelectListPref.setOnPreferenceChangeListener(
          (preference, newValue) -> {
            if (newValue instanceof HashSet
                && !((HashSet) newValue).contains(getString(R.string.exif_tag_location))) {
    ...
    betaTesterPreference.setOnPreferenceClickListener(
        preference -> {
          Utils.handleWebUrl(
              getActivity(), Uri.parse(getResources().getString(R.string.beta_opt_in_link)));
          return true;
        });
    ...
    if (defaultKvStore.getBoolean("login_skipped", false)) {
      ...
      SwitchPreference displayNearbyCardView =
          (SwitchPreference) findPreference("displayNearbyCardView");
      SwitchPreference displayLocationPermissionForCardView =
          (SwitchPreference) findPreference("displayLocationPermissionForCardView");
      SwitchPreference displayCampaignsCardView =
          (SwitchPreference) findPreference("displayCampaignsCardView");
   ...
   * Adds preference changed listener and saves value choosen by user to shared preferences to
   * remember later
   */
  ...
  private List<Language> getLanguagesSupportedByDevice() {
    ...
    Collections.sort(
        languages,
        (language, t1) ->
            language.getLocale().getDisplayName().compareTo(t1.getLocale().getDisplayName()));
    return languages;
  }
            ...
            new BasePermissionListener() {
              ...
              public void onPermissionGranted(PermissionGrantedResponse response) {
                ViewUtil.showLongToast(
                    getActivity(), getResources().getString(R.string.log_collection_started));
              }
            })
        .check();
  }
