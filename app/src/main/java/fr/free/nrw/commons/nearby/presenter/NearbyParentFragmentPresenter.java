...
public class NearbyParentFragmentPresenter
  ...
  private static final NearbyParentFragmentContract.View DUMMY =
      (NearbyParentFragmentContract.View)
          Proxy.newProxyInstance(
              ...
              new Class[] {NearbyParentFragmentContract.View.class},
              (proxy, method, args) -> {
  ...
  /**
   * Nearby updates takes time, since they are network operations. During update time, we don't want
   * to get any other calls from user. So locking nearby.
   *
  ...
  public void updateMapAndList(LocationServiceManager.LocationChangeType locationChangeType) {
    ...
    /**
     * Significant changed - Markers and current location will be updated together Slightly changed
     * - Only current position marker will be updated
     */
    if (locationChangeType.equals(LOCATION_SIGNIFICANTLY_CHANGED)
      ...
      if (!nearbyParentFragmentView
          .isSearchThisAreaButtonVisible()) { // Do not track users position if the user is checking
        // around
        nearbyParentFragmentView.recenterMap(curLatLng);
      }
    }
  ...
  public void updateMapMarkers(
      NearbyController.NearbyPlacesInfo nearbyPlacesInfo,
      Marker selectedMarker,
      boolean shouldTrackPosition) {
    if (null != nearbyParentFragmentView) {
      List<NearbyBaseMarker> nearbyBaseMarkers =
          NearbyController.loadAttractionsFromLocationToBaseMarkerOptions(
              nearbyPlacesInfo.curLatLng, // Curlatlang will be used to calculate distances
    ...
    for (int i = 0; i < nearbyBaseMarkers.size(); i++) {
      ...
      NearbyController.markerLabelList.add(
          new MarkerPlaceGroup(
              ...
              bookmarkLocationDao.findBookmarkLocation(nearbyBaseMarkers.get(i).getPlace()),
  ...
  public boolean searchCloseToCurrentLocation() {
    ...
    double distance =
        LocationUtils.commonsLatLngToMapBoxLatLng(nearbyParentFragmentView.getCameraTarget())
  ...
  public boolean areLocationsClose(LatLng cameraTarget, LatLng lastKnownLocation) {
    double distance =
        LocationUtils.commonsLatLngToMapBoxLatLng(cameraTarget)
