...
public class UploadClient {
  ...
  @Inject
  public UploadClient(
      UploadInterface uploadInterface, @Named(NAMED_COMMONS_CSRF) CsrfTokenClient csrfTokenClient) {
  ...
  Observable<UploadResult> uploadFileToStash(
    ...
    RequestBody requestBody =
        RequestBody.create(
            MediaType.parse(FileUtils.getMimeType(context, Uri.parse(file.getPath()))), file);

    CountingRequestBody countingRequestBody =
        new CountingRequestBody(
            requestBody,
            (bytesWritten, contentLength) ->
                notificationUpdater.onProgress(bytesWritten, contentLength));
    ...
    try {
      ...
      return uploadInterface
          .uploadFileToStash(fileNameRequestBody, tokenRequestBody, filePart)
  ...
  Observable<UploadResult> uploadFileFromStash(
      Context context, Contribution contribution, String uniqueFileName, String fileKey) {
    try {
      return uploadInterface
          .uploadFileFromStash(
              ...
              uniqueFileName,
              fileKey)
          .map(uploadResponse -> uploadResponse.getUpload());
    } catch (Throwable throwable) {
      throwable.printStackTrace();
      return Observable.error(throwable);
    }
