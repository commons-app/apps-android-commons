...
public class ImageProcessingService {
  ...
  Single<Integer> validateImage(UploadModel.UploadItem uploadItem, boolean checkTitle) {
    ...
    Single<Integer> itemTitle =
        checkTitle ? validateItemTitle(uploadItem) : Single.just(ImageUtils.IMAGE_OK);
    Single<Integer> checkFBMD = checkFBMD(filePath);
    Single<Integer> checkEXIF = checkEXIF(filePath);

    Single<Integer> zipResult =
        Single.zip(
            duplicateImage,
            wrongGeoLocation,
            darkImage,
            ...
            (duplicate, wrongGeo, dark, title) -> {
              Timber.d(
                  "Result for duplicate: %d, geo: %d, dark: %d, title: %d",
                  duplicate, wrongGeo, dark, title);
   ...
   * does not have any Exif metadata, then it was likely downloaded from the internet, and is
   * probably not an original work by the user. We detect these kinds of images by looking for the
   * presence of some basic Exif metadata.
   */
  ...
  /**
   * Checks item title - empty title - existing title
   *
   * @param uploadItem
   * @return
   */
  private Single<Integer> validateItemTitle(UploadModel.UploadItem uploadItem) {
    ...
    return mediaClient
        .checkPageExistsUsingTitle("File:" + uploadItem.getFileName())
        .map(
            doesFileExist -> {
    ...
    return Single.fromCallable(() -> filePath)
        ...
        .flatMap(
            geoLocation -> {
              ...
              return imageUtilsWrapper.checkImageGeolocationIsDifferent(
                  geoLocation, place.getLocation());
            });
  }
}
