...
public class UploadMediaPresenter implements UserActionListener, SimilarImageInterface {
  ...
  private static final UploadMediaDetailsContract.View DUMMY =
  ...
  /**
   * Receives the corresponding uploadable file, processes it and return the view with and uplaod
   * item
   *
  ...
  public void receiveImage(UploadableFile uploadableFile, String source, Place place) {
    ...
    Disposable uploadItemDisposable =
        repository
  ...
  private void checkNearbyPlaces(UploadItem uploadItem) {
    Disposable checkNearbyPlaces =
        Observable.fromCallable(
            ...
            .observeOn(mainThreadScheduler)
            .subscribe(
                place -> view.onNearbyPlaceFound(uploadItem, place),
                throwable -> Timber.e(throwable, "Error occurred in processing images"));
  ...
  public void verifyImageQuality(UploadItem uploadItem, boolean validateTitle) {
    view.showProgress(true);
    Disposable imageQualityDisposable =
        repository
  ...
  public void fetchPreviousTitleAndDescription(int indexInViewFlipper) {
    ...
    if (null != previousUploadItem) {
      view.setTitleAndDescription(
          previousUploadItem.getTitle().getTitleText(), previousUploadItem.getDescriptions());
    } else {
  ...
  public void handleBadImage(Integer errorCode) {
    ...
    if (errorCode
        >= 8) { // If location of image and nearby does not match, then set shared preferences to
      // disable wikidata edits
