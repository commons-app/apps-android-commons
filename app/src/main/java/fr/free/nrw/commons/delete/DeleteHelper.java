...
public class DeleteHelper {
  ...
  public Single<Boolean> makeDeletion(Context context, Media media, String reason) {
    viewUtil.showShortToast(
        context, "Trying to nominate " + media.getDisplayTitle() + " for deletion");
  ...
  private Observable<Boolean> delete(Media media, String reason) {
    ...
    return pageEditClient
        .prependEdit(media.getFilename(), fileDeleteString + "\n", summary)
        .flatMap(
            result -> {
              if (result) {
                return pageEditClient.edit(
                    "Commons:Deletion_requests/" + media.getFilename(),
                    subpageString + "\n",
                    summary);
              }
        ...
        .flatMap(
            result -> {
              if (result) {
                return pageEditClient.appendEdit(
                    "Commons:Deletion_requests/" + date, logPageString + "\n", summary);
              }
              throw new RuntimeException("Failed to nominate for deletion");
            })
        .flatMap(
            result -> {
              if (result) {
                return pageEditClient.appendEdit(
                    "User_Talk:" + username, userPageString + "\n", summary);
              }
              throw new RuntimeException("Failed to nominate for deletion");
            });
  }
  ...
  private boolean showDeletionNotification(Context context, Media media, boolean result) {
    ...
    if (result) {
      ...
      message =
          context.getString(
              (R.string.delete_helper_show_deletion_message_if), media.getDisplayTitle());
    } else {
    ...
    notificationHelper.showNotification(
        context, title, message, NOTIFICATION_DELETE, browserIntent);
    return result;
  }
  ...
  public void askReasonAndExecute(
      Media media,
      Context context,
      String question,
    ...
    alert.setPositiveButton(
        ...
        (dialogInterface, i) -> {
          String reason =
              context.getString(R.string.delete_helper_ask_alert_set_positive_button_reason) + " ";
          ...
          Single.defer(
                  (Callable<SingleSource<Boolean>>) () -> makeDeletion(context, media, finalReason))
    ...
    alert.setNegativeButton(
        context.getString(R.string.cancel), (dialog, which) -> reviewCallback.onFailure());
