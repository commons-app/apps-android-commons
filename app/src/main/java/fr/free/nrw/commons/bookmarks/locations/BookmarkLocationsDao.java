...
public class BookmarkLocationsDao {
  ...
  private ContentValues toContentValues(Place bookmarkLocation) {
    ...
    cv.put(
        BookmarkLocationsDao.Table.COLUMN_WIKIPEDIA_LINK,
        bookmarkLocation.siteLinks.getWikipediaLink().toString());
    cv.put(
        BookmarkLocationsDao.Table.COLUMN_WIKIDATA_LINK,
        bookmarkLocation.siteLinks.getWikidataLink().toString());
    cv.put(
        BookmarkLocationsDao.Table.COLUMN_COMMONS_LINK,
  ...
  public static class Table {
    ...
    public static void onUpdate(SQLiteDatabase db, int from, int to) {
      ...
      if (from == 10 && to == 11) {
        from++;
        // This is safe, and can be called clean, as we/I do not remember the appropriate version
        // for this
