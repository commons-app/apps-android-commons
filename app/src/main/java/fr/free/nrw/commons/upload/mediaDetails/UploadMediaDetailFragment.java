...
public class UploadMediaDetailFragment extends UploadBaseFragment implements
  ...
  private void init() {
    tvTitle.setText(getString(R.string.step_count, callback.getIndexInViewFlipper(this) + 1,
        callback.getTotalNumberOfSteps(), getString(R.string.media_detail_step_title).trim()));
  ...
  private void initRecyclerView() {
    uploadMediaDetailAdapter = new UploadMediaDetailAdapter(
        defaultKvStore.getString(Prefs.KEY_LANGUAGE_VALUE, "").trim());
  ...
  private void showInfoAlert(int titleStringID, int messageStringId) {
    DialogUtil.showAlertDialog(getActivity(), getString(titleStringID).trim(), getString(messageStringId).trim(),
  ...
  public void showMessage(String message, int colorResourceId) {
    ViewUtil.showLongToast(getContext(), message.trim());
  }
