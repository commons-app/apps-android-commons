...
public class CategoryImagesListFragment extends DaggerFragment {
  ...
  /**
   * Initializes the UI elements for the fragment Setup the grid view to and scroll listener for it
   */
  ...
  /**
   * Query continue values determine the last page that was loaded for the particular keyword This
   * method resets those values, so that the results can be queried from the first page itself
   *
   * @param keyword
   */
  ...
  /**
   * Checks for internet connection and then initializes the grid view with first 10 images of that
   * category
   */
  ...
  private void setAdapter(List<Media> mediaList) {
    gridAdapter = new GridViewAdapter(this.getContext(), R.layout.layout_category_images,
        mediaList);
    gridView.setAdapter(gridAdapter);
  }

  /**
   * Sets the scroll listener for the grid view so that more images are fetched when the user
   * scrolls down Checks if the category has more images before loading Also checks whether images
   * are currently being fetched before triggering another request
   */
  private void setScrollListener() {
    gridView.setOnScrollListener(new AbsListView.OnScrollListener() {
      ...
      public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount,
          int totalItemCount) {
        if (hasMoreImages && !isLoading && (firstVisibleItem + visibleItemCount + 1
            >= totalItemCount)) {
          isLoading = true;
          fetchMoreImages();
        }
  ...
  /**
   * This method is called when viewPager has reached its end. Fetches more images for the category
   * and adds it to the grid view and viewpager adapter
   */
  ...
  /**
   * Handles the success scenario On first load, it initializes the grid view. On subsequent loads,
   * it adds items to the adapter
   *
  ...
  /**
   * If caption is available for the image, then modify grid adapter to show captions
   */
