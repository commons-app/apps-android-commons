...
public class UploadModel {
  ...
  private static UploadItem DUMMY =
      new UploadItem(Uri.EMPTY, Uri.EMPTY, "", "", GPSExtractor.DUMMY, null, -1L, "") {};
  ...
  @Inject
  UploadModel(
      @Named("licenses") List<String> licenses,
  ...
  Observable<UploadItem> preProcessImages(
  ...
  public Observable<UploadItem> preProcessImage(
  ...
  private UploadItem getUploadItem(
      UploadableFile uploadableFile,
      Place place,
      String source,
      SimilarImageInterface similarImageInterface) {
    fileProcessor.initFileDetails(
        Objects.requireNonNull(uploadableFile.getFilePath()), context.getContentResolver());
    UploadableFile.DateTimeWithSource dateTimeWithSource =
    ...
    GPSExtractor gpsExtractor =
        fileProcessor.processFileCoordinates(similarImageInterface, context);
            ...
            item -> {
              Contribution contribution =
                  new Contribution(
                      ...
                      null,
                      item.getFileName(),
                      Description.formatList(item.descriptions),
                      -1,
                      null,
                      null,
                      sessionManager.getAuthorName(),
                      CommonsApplication.DEFAULT_EDIT_SUMMARY,
                      item.gpsCoords.getCoords());
              if (item.place != null) {
              ...
              Timber.d(
                  ...
                  item.getCreatedTimestamp(), new Date(item.getCreatedTimestamp()));
              if (item.createdTimestamp != -1L) {
                ...
                // Set the date only if you have it, else the upload service is gonna try it the
                // other way
              }
              return contribution;
            });
  ...
  public static class UploadItem {
    ...
    @SuppressLint("CheckResult")
    UploadItem(
        Uri originalContentUri,
        Uri mediaUri,
        String mimeType,
        String source,
        GPSExtractor gpsCoords,
        Place place,
    ...
    public String getFileName() {
      return title != null ? Utils.fixExtension(title.toString(), getFileExt()) : null;
    }
