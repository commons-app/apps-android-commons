...
public class UploadableFile implements Parcelable {
  public static final Creator<UploadableFile> CREATOR =
      new Creator<UploadableFile>() {
        @Override
  ...
  private DateTimeWithSource getFileCreatedDateFromCP(Context context) {
    try {
      ...
      // Content provider contracts for opening gallery from the app and that by sharing from
      // gallery from outside are different and we need to handle both the cases
      int lastModifiedColumnIndex =
          cursor.getColumnIndex("last_modified"); // If gallery is opened from in app
      ...
      cursor.moveToFirst();
      return new DateTimeWithSource(
          cursor.getLong(lastModifiedColumnIndex), DateTimeWithSource.CP_SOURCE);
    } catch (Exception e) {
      return null; //// Could not fetch last_modified
    }
  }
