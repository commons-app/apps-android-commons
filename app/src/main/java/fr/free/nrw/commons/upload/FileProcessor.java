...
public class FileProcessor implements Callback {
  ...
  GPSExtractor processFileCoordinates(
      SimilarImageInterface similarImageInterface, Context context) {
  ...
  private Set<String> getExifTagsToRedact(Context context) {
    ...
    Set<String> redactTags =
        new HashSet<>(
            Arrays.asList(context.getResources().getStringArray(R.array.pref_exifTag_values)));
  ...
  private void findOtherImages(SimilarImageInterface similarImageInterface) {
    ...
    long timeOfCreation =
        new File(filePath).lastModified(); // Time when the original image was created
    ...
    for (File file : files) {
      if (file.lastModified() - timeOfCreation <= (120 * 1000)
        ...
        if (tempImageObj != null) {
          Timber.d(
              "not null fild EXIF"
                  + tempImageObj.imageCoordsExists
                  + " coords"
                  + tempImageObj.getCoords());
  ...
  private void useImageCoords() {
    if (decimalCoords != null) {
      ...
      // If no categories found in cache, call MediaWiki API to match image coords with nearby
      // Commons categories
      if (catListEmpty) {
        compositeDisposable.add(
            apiCall
                .request(decimalCoords)
