 ...
 *
 * <p>This includes things like category lists and multilingual descriptions, which are not
 * intrinsic to the media and may change due to editing.
 */
@Singleton
public class MediaDataExtractor {
  ...
  /**
   * Simplified method to extract all details required to show media details. It fetches media
   * object, deletion status and talk page for the filename
   *
  ...
  public Single<Media> fetchMediaDetails(String filename) {
    ...
    Single<Boolean> pageExistsSingle =
    ...
    return Single.zip(
        ...
        pageExistsSingle,
        discussionSingle,
        (media, deletionStatus, discussion) -> {
  ...
  private Single<String> getDiscussion(String filename) {
    return mediaClient
        .getPageHtml(filename.replace("File", "File talk"))
        .map(
            discussion ->
                HtmlCompat.fromHtml(discussion, HtmlCompat.FROM_HTML_MODE_LEGACY).toString())
        .onErrorReturn(
            throwable -> {
