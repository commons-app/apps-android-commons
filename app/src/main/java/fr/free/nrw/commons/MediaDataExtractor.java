...
public class MediaDataExtractor {
   ...
   * object, deletion status, talk page and captions for the filename
   *
  ...
  @NotNull
  private Media combineToMedia(final Media media, final Boolean deletionStatus,
   ...
   * @return caption for the image in user's locale Ex: "a nice painting" (english locale) and "No
   * Caption" in case the caption is not available for the image
   */
  ...
  private Single<String> getDiscussion(final String filename) {
    return mediaClient.getPageHtml(filename.replace("File", "File talk"))
        .map(discussion -> HtmlCompat.fromHtml(discussion, HtmlCompat.FROM_HTML_MODE_LEGACY)
            .toString())
